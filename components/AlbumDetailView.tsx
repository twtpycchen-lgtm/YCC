    ...
    <div className="animate-reveal space-y-10 md:space-y-16 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 pb-6 border-b border-white/5">
        ...
        <div className="flex items-center gap-3">
          <button onClick={handleShare} className="relative px-6 py-2 rounded-full border border-white/5 text-gray-500 hover:text-white hover:border-white/20 transition-all uppercase text-[8px] tracking-[0.2em] font-black">
            Share Masterpiece
            {showCopyMsg && (
              <span className="absolute -top-12 left-1/2 -translate-x-1/2 text-[9px] text-black bg-[#d4af37] px-5 py-2 rounded-full shadow-[0_10px_30px_rgba(212,175,55,0.3)] animate-bounce whitespace-nowrap font-black">Link Copied</span>
            )}
          </button>
          {isCuratorMode && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-20 items-start">
        <div className="lg:col-span-4 space-y-12 lg:sticky lg:top-32">
          <div className="relative rounded-[2.5rem] overflow-hidden shadow-[0_30px_70px_rgba(0,0,0,0.8)] border border-white/10 bg-black group aspect-square">
            ...
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-700">
               <button onClick={onPlayAll} className="w-20 h-20 bg-[#d4af37] text-black rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(212,175,55,0.4)]">
        ...
        <div className="lg:col-span-8">
          <div className="mb-14">
            <h2 className="text-3xl md:text-5xl font-luxury mb-6 tracking-tight text-white leading-[1.2]">{album.title}</h2>
          ...
          <div className="mb-14">
             <button onClick={onPlayAll} className="group relative flex items-center gap-6 py-5 px-12 rounded-full border-2 border-[#d4af37]/40 bg-gradient-to-r from-[#d4af37]/10 to-transparent hover:from-[#d4af37]/20 hover:border-[#d4af37] transition-all shadow-xl">
              <div className="w-12 h-12 bg-[#d4af37] rounded-full flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-black ml-1" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" /></svg>
              </div>
              <div className="flex flex-col items-start">
                <span className="text-xl md:text-2xl font-luxury text-white tracking-[0.2em]">全部播放</span>
                <span className="text-[9px] uppercase tracking-[0.5em] text-[#d4af37] font-black">Start Archive Session</span>
              </div>
            </button>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2.5">
              {visibleTracks.map((track, idx) => {
                ...
                return (
                  <div key={track.id} onClick={() => onPlayTrack(track)} className={`group flex items-center justify-between p-5 md:p-6 rounded-[1.5rem] transition-all cursor-pointer border ${isActive ? 'bg-[#d4af37]/10 border-[#d4af37]/30' : 'hover:bg-white/[0.03] border-transparent'}`}>
                    <div className="flex items-center gap-6 md:gap-10 flex-grow min-w-0">
                      <span className="text-[10px] text-gray-800 font-mono w-6 text-center">{idx + 1}</span>
                      <div className="min-w-0 flex-grow">
                        <h5 className={`text-base md:text-xl font-medium tracking-wide transition-all ${isActive ? 'text-[#d4af37]' : 'text-gray-300 group-hover:text-white'}`}>
                          {track.title || track.originalTitle}
                        </h5>
                        {/* 放大「人工 Key-in 歌名」(remarks) 字體 */}
                        {track.remarks && (
                          <div className="text-xs md:text-sm uppercase tracking-[0.15em] text-gray-500 mt-2 font-bold group-hover:text-[#d4af37]/80 transition-colors">
                            <span className="opacity-40 mr-2 text-[8px]">Ref:</span>{track.remarks}
                          </div>
                        )}
                      </div>
                    </div>
                    <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center transition-all border ${isActive && isPlaying ? 'bg-[#d4af37] border-[#d4af37] text-black scale-105' : 'bg-transparent border-white/10 text-white'}`}>
            ...
            {hasManyTracks && (
              <div className="pt-10 flex justify-center">
                <button onClick={() => setIsExpanded(!isExpanded)} className="group flex flex-col items-center gap-3 py-3 px-10 rounded-full border border-white/10 hover:border-[#d4af37]/40 transition-all">
                  <span className="text-[9px] uppercase tracking-[0.5em] font-black text-gray-700 group-hover:text-[#d4af37]">
                    {isExpanded ? 'Collapse Program' : 'Reveal More'}
                  </span>
                  <div className={`transition-transform ${isExpanded ? 'rotate-180' : ''}`}>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-800 group-hover:text-[#d4af37]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                  </div>
                </button>
              </div>
            )}
